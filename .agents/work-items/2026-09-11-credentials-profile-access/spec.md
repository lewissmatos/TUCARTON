...
- Phone ownership verification, password recovery, passkey, email, Google, or external identity-provider login.
- Account recovery for accounts created by the retired code-based development flow without a passcode.
...
- `R-05`: The code-based access endpoints, fake verification provider, fake-code test, and mobile code-entry UI must be removed from the active application flow.
...
- `AC-04`: Given the active mobile access flow, then no one-time-code input, development code, or code-verification endpoint is used or displayed.
...
| AC-04 | API/mobile static review | Removed fake-provider and code-verification endpoint/UI references. |
...
| Risk | Development code-only accounts have no passcode and cannot log in through this replacement flow. | Accept for current development data; later recovery decision. |
...
| --- | --- | --- | --- |
| Approved | Senior Developer | 2026-09-11 | Replace one-time codes with name, phone, and numeric passcode registration/login; support client lookup by code or phone. |
...
Risks / blockers: Legacy code-only development accounts require future recovery support; no external provider is required.
