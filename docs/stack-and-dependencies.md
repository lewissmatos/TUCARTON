...
| Mobile authentication        | Local passcode sessions + `expo-secure-store`              | The API hashes passcodes and issues TuCartón-owned revocable sessions; mobile stores session credentials securely. |
...
| `@tucarton/mobile`       | `expo@57.0.21`, `expo-blur@~57.0.2`, `expo-camera@57.0.4`, `expo-router@57.0.20`, `expo-secure-store@57.0.3`, `expo-sqlite@57.0.2`, `react@19.2.3`, `react-native@0.86.3`                                                          |
| `@tucarton/api`          | `@nestjs/common@12.0.1`, `@nestjs/core@12.0.1`, `@nestjs/platform-express@12.0.1`, `@nestjs/swagger@12.0.1`, `jose@6.2.12`, `reflect-metadata@0.2.2`, `rxjs@7.8.2`; development: `@nestjs/cli@12.0.0`, `@nestjs/schematics@12.0.0` |
...
For a physical device on the same Wi-Fi network, set `EXPO_PUBLIC_API_URL` in an untracked `apps/mobile/.env` to the Mac's Bonjour hostname or a reachable development API URL. Expo Go can run the current mobile app.
